...
const BASE_URL =
  ...
  const fromCurr = document.body.querySelector(".from select");
  const toCurr = document.body.querySelector(".to select");
  ...
  button.addEventListener('click',async (evnt)=>{
    ...
    if(amountValue<0 || amountValue==="" || amountValue==0){
      amountValue=1;
      amount.value="1";
    }
    ...
    msg.innerHTML=`<p>${amountValue} ${fromCurr.value} = ${finalAmount} in ${toCurr.value} </p>`
